...
public class Bisshop extends Figure {
    ...
    void Move(Chessboard b, String position) {
        ...
        if (moveRow <0 && moveRow >= b.table && moveColumn <0 && moveColumn > b.table) {
            ...
            System.out.println("Cannot move " + b.board[moveRow][moveColumn].getName() + " because the position is occupied.");
                    return;
            ...
            while (checkRow != moveRow && checkColumn != moveColumn && checkColumn>0&&checkRow>0) {
                if (b.board[checkRow][checkColumn] != null && b.board[checkRow][checkColumn].isWhite()==this.isWhite) {
                    System.out.println("Cannot move " + this.getName() + " to " + position + " because the path "+b.board[checkRow][checkColumn].name+ " is blocked.");
                    return;
                }
                if (b.board[checkRow][checkColumn] != null ) {
                    System.out.println("Cannot move " + this.getName() + " to " + position + " because the path is blocked.");
                    return;
                }
